import { normalizeAuthSession } from "@/shared/api/client";
import { env } from "@/shared/config/env";
import { useAuth } from "@/shared/context/AuthContext";
import { Button } from "@hisabkit/ui/components/Button";
import { Card } from "@hisabkit/ui/components/Card";
import { Input } from "@hisabkit/ui/components/Input";
import { type CredentialResponse, GoogleLogin } from "@react-oauth/google";
import { ArrowLeft, Languages, Loader2, LockKeyhole, ShieldCheck, Sparkles } from "lucide-react";
import type { FormEvent } from "react";
import { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { loginWithGoogleCredential, loginWithUsernamePassword } from "../services/authApi";
...
export const Login = () => {
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        apiError.response?.data?.message || t("login.error", "Invalid username or password"),
      );
    } finally {
      setIsLoading(false);
    }
    ...
    try {
      ...
      setError(
        apiError.response?.data?.message ||
          t("login.google_failed", "Google sign-in failed. Please try again."),
      );
    } finally {
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              >
                ...
                {i18n.resolvedLanguage === "hi"
                  ? t("login.switch_to_english", "English")
                  : t("login.switch_to_hindi", "Hindi")}
              </Button>
            </div>
            ...
            <div className="mt-10 space-y-5">
              ...
              <div className="space-y-4">
                <h1 className="text-4xl font-black tracking-tight text-white sm:text-5xl">
                  {t("login.hero_title", "Login to your Shop")}
                </h1>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                ...
                <h2 className="mt-4 text-sm font-bold text-white">
                  {t("login.feature_data_title", "Isolated Shop Data")}
                </h2>
                <p className="mt-2 text-sm leading-6 text-slate-300">
                  {t(
                    "login.feature_data_description",
              ...
              <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                ...
                <h2 className="mt-4 text-sm font-bold text-white">
                  {t("login.feature_i18n_title", "Multilingual Support")}
                </h2>
                <p className="mt-2 text-sm leading-6 text-slate-300">
                  {t(
                    "login.feature_i18n_description",
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-3xl font-black tracking-tight text-slate-900 dark:text-white">
                  {t("login.shopkeeper_login", "Shopkeeper Login")}
                </h2>
              </div>
            ...
            <form onSubmit={handleLogin} className="mt-8 space-y-5">
              ...
              >
                ...
                {isLoading
                  ...
                  : t("login.sign_in_password", "Sign in with password")}
              </Button>
            </form>
