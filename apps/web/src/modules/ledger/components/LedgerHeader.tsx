...
import { Download, PieChart, Upload, UserPlus, Wallet } from "lucide-react";
...
export function LedgerHeader({
  ...
  return (
    <div className="sticky top-0 z-30 glass-card rounded-2xl p-3 px-5 reveal">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1">
            <Button
              variant="ghost"
              size="icon"
              className="h-9 w-9 rounded-xl text-slate-500 hover:text-indigo-600"
              onClick={onImport}
              title="Import CSV"
            >
      ...
      {showTotals && (
        <div className="mt-8 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-2xl bg-amber-500/5 border border-amber-500/10 p-5 shadow-sm">
            ...
            <p className="text-2xl font-black text-amber-700 dark:text-amber-300">{overdueCount}</p>
          </div>
        </div>
      )}
    </div>
