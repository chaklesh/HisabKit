...
const DEFAULT_TEMPLATE =
  "Hi {{customerName}}, this is {{businessName}}. Your outstanding balance is {{balance}} ({{balanceType}}). Please settle when convenient. Thank you!";
...
export function RemindersSection({ businessData }: RemindersSectionProps) {
  ...
  useEffect(() => {
    if (businessData) {
      ...
      setWhatsappTemplate(
        businessData.whatsappTemplate || businessData.smsTemplate || DEFAULT_TEMPLATE,
      );
    }
  }, [businessData]);
  ...
  const textareaClasses =
    "w-full rounded-lg border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 font-mono";
          ...
          <div>
            ...
            <CardDescription>
              {t(
                ...
                "Customize messages sent to customers via SMS and WhatsApp",
              )}
            </CardDescription>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="sms-template" className={labelClasses}>
                {t("settings.reminders.sms_template", "SMS template")}
              </label>
            ...
            <p className="text-xs text-muted-foreground">
              {smsTemplate.length}/160 characters (SMS length)
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="whatsapp-template" className={labelClasses}>
                {t("settings.reminders.whatsapp_template", "WhatsApp template")}
              </label>
            ...
            <p className="text-xs text-muted-foreground">
              {t(
                "settings.reminders.whatsapp_note",
                "WhatsApp allows longer messages and media support",
              )}
            </p>
          </div>
