...
- Work only inside the intended worktree; do not mix edits across sibling worktrees.
- Start each application from its own root.
- Treat docs as part of the deliverable whenever architecture, config, contracts, or workflow expectations change.
- Prefer small, reversible increments with validation evidence after each meaningful slice.
...
- Backend work should also follow `docs/backend-engineering-playbook.md`.
- Frontend work should also follow `docs/frontend-engineering-playbook.md`.
- Mobile work should also follow `docs/mobile-engineering-playbook.md`.
- Cross-platform visual and UX work should also follow `docs/brand-theme-standards.md`.
...
Recommended before large mobile changes:
...
- Update the closest existing doc instead of creating parallel policy files.
- Add a new playbook only when a stack lacks one and the guidance cannot fit a current doc.
- Keep claims about implementation status truthful; remove drift when the codebase no longer matches a doc.
- Link validation evidence in `docs/agent-handoff-log.md` for meaningful work packages.
