...
class AttachmentStorageServiceTest {
  ...
  void setUp() {
    ...
    tenant = Tenant.builder().id(tenantId).attachmentQuotaMb(10).maxAttachmentFileSizeMb(5).build();
  }
  ...
  void testUploadAttachment_QuotaExceeded_ThrowsException() {
    ...
    when(attachmentRepository.totalStorageUsedByTenant(tenantId))
        .thenReturn(11L * 1024 * 1024); // 11MB used, cap 10MB

    assertThrows(
        StorageException.class, () -> storageService.uploadAttachment(tenantId, txnId, file));
  }
  ...
  void testDeleteAttachment_Success() {
    ...
    when(attachmentRepository.findByIdAndTenantId(attachmentId, tenantId))
