...
public class GoogleSocialAuthService {
  ...
  public AuthResponse authenticate(GoogleSocialLoginRequest request) {
    ...
    if (StringUtils.hasText(googleSubject)
      ...
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "This Google account does not match your registered account.");
    }
  ...
  private GoogleProfile verifyToken(String idToken) {
    if (!StringUtils.hasText(properties.getGoogle().getClientId())) {
      throw new ResponseStatusException(
          HttpStatus.SERVICE_UNAVAILABLE, "Google social login is not configured");
    }
    ...
    try {
      ResponseEntity<Map> response =
          restTemplate.getForEntity(
              "https://oauth2.googleapis.com/tokeninfo?id_token={idToken}", Map.class, idToken);
      ...
      if (verifierException != null) {
        log.warn(
            "Google token verification failed in verifier and tokeninfo fallback",
            verifierException);
      }
      throw new ResponseStatusException(
          HttpStatus.UNAUTHORIZED, "Unable to verify Google ID token", ex);
    }
  }
  ...
  private record GoogleProfile(
      String email, String name, String subject, String mobile, String picture) {}
}
