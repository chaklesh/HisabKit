...
public class TenantService {
  ...
  public Tenant createTenant(CreateTenantRequest request) {
    ...
    Tenant tenant =
        Tenant.builder()
            ...
            .smsTemplate(
                defaultValue(
                    request.getSmsTemplate(),
                    "Hi {{customerName}}, your balance is {{balance}} ({{balanceType}}) with {{businessName}}."))
            .whatsappTemplate(
                defaultValue(
                    request.getWhatsappTemplate(),
                    "Hi {{customerName}}, this is a reminder from {{businessName}}. Your balance is {{balance}} ({{balanceType}})."))
    ...
    auditService.logMutation(
        ...
        "CREATE",
        tenant.getId(),
        java.util.Map.of("name", tenant.getName(), "admin", adminUser.getUsername()),
        tenant.getName());
    return tenant;
  }
  ...
  public void deleteTenant(UUID tenantId) {
    ...
    // Log before deletion to ensure we have the data
    auditService.logMutation(
        "TENANT",
        tenantId,
        "DELETE",
        tenantId,
        java.util.Map.of("name", tenant.getName()),
        tenant.getName());
                  ...
                  .forEach(
                      txn -> {
                        attachmentRepository.deleteAll(
                            attachmentRepository.findByTransactionId(txn.getId()));
                      });
              transactionRepository.deleteAll(
                  transactionRepository.findByTenantIdAndCustomerId(tenantId, customer.getId()));
            });
  ...
  private void applyTenantUpdate(Tenant tenant, UpdateTenantRequest request) {
    ...
    tenant.setAttachmentQuotaMb(
        defaultIntValue(request.getAttachmentQuotaMb(), tenant.getAttachmentQuotaMb()));
    tenant.setMaxAttachmentFileSizeMb(
        defaultIntValue(request.getMaxAttachmentFileSizeMb(), tenant.getMaxAttachmentFileSizeMb()));
    tenant.setAttachmentRetentionDays(
        defaultIntValue(request.getAttachmentRetentionDays(), tenant.getAttachmentRetentionDays()));
  }
