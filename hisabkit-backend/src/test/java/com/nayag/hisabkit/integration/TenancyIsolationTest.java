...
class TenancyIsolationTest {
    ...
    void setUp() {
        ...
        User userA = userRepository.save(User.builder().username("userA").passwordHash("hash").role("USER").tenant(tenantA).isActive(true).build());
        tokenA = "Bearer " + jwtUtils.generateToken(userA.getUsername(), tenantA.getId().toString(), userA.getRole());

        // Create Tenant B
        tenantB = tenantRepository.save(Tenant.builder().name("Tenant B").slug("tenant-b").build());
        User userB = userRepository.save(User.builder().username("userB").passwordHash("hash").role("USER").tenant(tenantB).isActive(true).build());
        tokenB = "Bearer " + jwtUtils.generateToken(userB.getUsername(), tenantB.getId().toString(), userB.getRole());
    ...
    void testTenantACannotSeeTenantBData() throws Exception {
        mockMvc.perform(get("/api/ledger/customers")
