import { Save, UserCircle2, Camera, UploadCloud } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-[2rem] border border-slate-100 dark:border-slate-800">
        <div className="relative group cursor-pointer">
          <div className="w-24 h-24 rounded-[2rem] border-4 border-white dark:border-slate-800 shadow-xl overflow-hidden bg-slate-200 dark:bg-slate-800 flex items-center justify-center">
            {profile.avatarUrl ? (
              <img src={profile.avatarUrl} alt={profile.fullName || profile.username} className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-500" />
            ) : (
           ...
           <div className="flex items-center justify-center md:justify-start gap-2">
              <Button
                ...
                className={cn(
                  ...
                  avatarFile ? "bg-indigo-600 hover:bg-indigo-700" : "bg-slate-200 dark:bg-slate-800 text-slate-400"
                )}
              >
                <UploadCloud className="w-4 h-4 mr-2" />
                Upload
              </Button>
              {avatarFile && <span className="text-[10px] font-bold text-indigo-500 animate-pulse uppercase tracking-wider">File Selected</span>}
           </div>
        </div>
      </div>
